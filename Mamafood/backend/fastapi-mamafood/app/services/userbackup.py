...
async def create_user_account(data, session, background_tasks, role=0):
...
async def get_login_token(data, background_tasks, session):
    ...
    if not user.is_active  or not user.verified_at:
        db_user = session.query(User).filter(User.email == data.username).first()
        if not db_user.updated_at:
            from datetime import datetime
            db_user.updated_at = datetime.utcnow()
            session.commit()
        await send_account_verification_email(db_user, background_tasks)
...
async def resend_activation_link(data, background_tasks, session):
    user_exist = session.query(User).filter(User.email == data.email).first()

    if not user_exist:
        raise HTTPException(status_code=400, detail="User not found")

    if user_exist.verified_at:
        raise HTTPException(status_code=400, detail="Your account is already verified.")

    # Ensure updated_at is not None before calling send_account_verification_email
    if not user_exist.updated_at:
        from datetime import datetime
        user_exist.updated_at = datetime.utcnow()

    # Just reuse existing user object to send email
    await send_account_verification_email(user_exist, background_tasks=background_tasks)
    

    return {"detail": "Verification link has been resent to your email."}
