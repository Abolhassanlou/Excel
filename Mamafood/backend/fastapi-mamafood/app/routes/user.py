...
async def user_login(data: OAuth2PasswordRequestForm = Depends(), background_tasks: BackgroundTasks=None, session: Session = Depends(get_session)):
    return await user.get_login_token(data,background_tasks, session)
...
@guest_router.post("/resend-activation-link",status_code=status.HTTP_200_OK)
async def resend_activation(data: EmailRequest, background_tasks: BackgroundTasks, session: Session = Depends(get_session)):
    await user.resend_activation_link(data, background_tasks, session)
    return JSONResponse({"message": "A email with verification link has been sent to you."})